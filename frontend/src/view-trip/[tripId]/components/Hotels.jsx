...
function Hotel({ trip }) {
  const hotelList = trip?.tripData?.hotels || [];

  if (hotelList.length === 0) {
    return <p className="text-gray-600 mt-4">No hotel recommendations available.</p>;
  }

  // Generate Google Maps search URL from hotel name and address
        ...
        {hotelList.map((item, index) => (
          ...
          >
            ...
            <div className="p-4 space-y-1">
              ...
              <p className="text-sm text-gray-600">Price: {item.PricePerNight}</p>
