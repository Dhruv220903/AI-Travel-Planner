    ...
    <div className="my-10 px-4 md:px-10">
      ...
      {itinerary.map((day, index) => (
        <div key={index} className="mb-10 bg-white shadow-md rounded-xl p-6">
          <h3 className="text-xl font-bold text-orange-600 mb-2">Day {day.day}: {day.theme}</h3>

          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {Array.isArray(day?.places) &&
              day.places.map((activity, idx) => {
                const imageUrl =
                  activity.placeImageUrl || `https://source.unsplash.com/400x300/?tourism,place,${index}-${idx}`;
                return (
